...
describe("Test Add Quote", function(){
	it("Add a Quote", function(done){
		let quote = new Quote({
			...
			categories:["hạt giống tâm hồn","cách sống","giáo dục","hạnh phúc"],
		...
		quote.save(function(err, quote){
			...
			assert.equal(quote.categories.length,4);
	...
	it("Add list Quote", function(done){
		...
		function(err, results){
			...
			//create Quote
			let arrQuote = [];
			let numQuote = faker.random.number({min: 60, max: 100});
			for(let i = 0; i< numQuote; i++){
				...
				for(let i = 0; i< catArrLength; i++){
					arrCategory.push(arrCategories[faker.random.number({min:0,max:arrCategories.length-1})].name);
				}
				arrCategory = _.uniq(arrCategory);
